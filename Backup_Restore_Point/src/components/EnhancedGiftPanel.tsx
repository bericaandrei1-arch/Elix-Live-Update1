...
function useInView<T extends Element>(options?: IntersectionObserverInit) {
  ...
  // Stable reference to options to avoid unnecessary effect re-runs
  ...
  }, []); // Only re-run if element changes (ref logic handled by React) or if we want to support dynamic options (add deps)
...
export function EnhancedGiftPanel({ onSelectGift, userCoins, onRechargeSuccess }: GiftPanelProps) {
  ...
  // Memoize options to prevent infinite loop/performance issues
  const observerOptions = useMemo(() => ({ root: null, threshold: 0.05 }), []);
  const { ref: panelRef, inView } = useInView<HTMLDivElement>(observerOptions);
